...
export function StudyBoard() {
  ...
  return (
    <section className="space-y-4">
      <div className="grid gap-3 rounded-[1.7rem] border bg-card p-3 shadow-soft lg:grid-cols-[minmax(0,1fr)_auto_auto] lg:items-center">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border bg-primary text-primary-foreground shadow-glow">
            <GraduationCap className="h-5 w-5" />
          </div>
          <div className="min-w-0">
            <h2 className="truncate text-lg font-black tracking-[-0.035em] sm:text-xl">Organize temas e assuntos</h2>
            <p className="mt-0.5 text-xs font-medium text-muted-foreground sm:text-sm">Crie uma trilha como “IA” e marque cada assunto concluído.</p>
          </div>
        </div>
        ...
        <Button className="h-11 gap-2 shadow-glow sm:h-10" onClick={() => setCreateOpen(true)}>
      ...
      {isLoading ? (
        ...
        <Card className="border-dashed bg-card/82 shadow-soft">
          <CardContent className="flex min-h-72 flex-col items-center justify-center gap-3 p-8 text-center">
            <BookOpenCheck className="h-11 w-11 text-primary" />
            <div>
              <h3 className="text-lg font-black tracking-[-0.035em]">Nenhuma trilha criada ainda</h3>
...
function StudyTopicCard({ topic }: { topic: StudyTopic }) {
  ...
  return (
    <Card className="overflow-hidden bg-card/90 shadow-soft">
      <CardContent className="space-y-3 p-3 sm:p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="truncate text-2xl font-black tracking-[-0.05em]">{topic.title}</h3>
        ...
        <div className="space-y-2">
          ...
          <div className="h-2 overflow-hidden rounded-full bg-muted">
            <motion.div className="h-full rounded-full bg-primary" initial={{ width: 0 }} animate={{ width: `${topic.progressPercent}%` }} />
          </div>
        </div>
        ...
        <div className="scrollbar-soft max-h-64 space-y-2 overflow-y-auto pr-1">
          {topic.subtopics.length === 0 ? (
            ...
            topic.subtopics.map((subtopic) => (
              <div key={subtopic.id} className={cn("group rounded-2xl border bg-background/82 p-3 transition hover:border-primary/60 hover:bg-background", subtopic.completed && "opacity-70")}>
                <div className="flex items-start gap-3">
                  <button
                    className={cn("-ml-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition", subtopic.completed ? "text-[oklch(0.62_0.14_145)]" : "text-muted-foreground hover:text-foreground")}
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={cn("font-bold leading-tight tracking-[-0.015em]", subtopic.completed && "line-through")}>{subtopic.title}</p>
        ...
        <form className="grid gap-2 rounded-2xl border bg-background/62 p-2.5" onSubmit={form.handleSubmit(submitSubtopic)}>
...
function StudyMetric({ label, value, loading }: { label: string; value: string; loading: boolean }) {
  return (
    <div className="rounded-2xl border bg-background/72 p-2.5">
      <p className="truncate text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">{label}</p>
      {loading ? <Skeleton className="mt-2 h-6 w-12" /> : <p className="mt-1.5 text-2xl font-black tracking-[-0.045em]">{value}</p>}
    </div>
  );
}
